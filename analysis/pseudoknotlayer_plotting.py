...
for parser in parsers:
    for variant in ['canonical_only', 'all']:
        ...
        print(f"df['layers']: {df['num_of_layers']}")
        ...
        for multilayer in [True, False]:
            ...
            print(f"Saved pie chart for {parser} ({variant}, {'multilayer' if multilayer else 'single-layer'}) to {fn}")
            plt.close()
        ...
        plt.savefig(os.path.join(output_dir, parser, fn))
        print(f"Saved bar graph for {parser} ({variant}) to {fn}")
        plt.close()